...
def rmat_from_vectors(vec1, vec2):
    """
    Get the rotation matrix that aligns vec1 to vec2.
    Cribbed from https://stackoverflow.com/questions/45142959/calculate-rotation-matrix-to-align-two-vectors-in-3d-space

    Parameters:
        vec1 : 3D "source" vector
        vec2 : 3D "destination" vector

    Returns:

        np.matrix : A transform matrix (3x3) which when applied to vec1, aligns it with vec2.
    """
    ...
    if any(v):
        ...
        c = np.dot(a, b)
        s = np.linalg.norm(v)
        kmat = np.matrix([[0, -v[2], v[1]], [v[2], 0, -v[0]], [-v[1], v[0], 0]])
        return np.eye(3) + kmat + kmat @ kmat * ((1 - c) / (s ** 2))
    else:
        # Trivial case: cross of all zeros only occurs on identical directions
