...
def filter_points(ref_points, new_points, n_points=50, err_limit=10):
    ...
    """
    try:
        transform = np.matrix(cv2.estimateAffinePartial2D(new_points, ref_points)[0])
...
def detect_movement(ref,moved):
    """
    Attempt to auto-detect image movement between two images using sparse optical flow and return a
...
def phase_correlation_movement(ref,moved):
    """
    Attempt to auto-detect a simple rigid translation of an image relative to a reference image
    using Phase Correlation and return a MovementCorrection object representing the movement.
    ...
    if c < 0.2:
        raise DetectionFailedError('Could not determine image movement from automatically: correlation coefficient too small ({:.2f}). Consider using manual movement correction instead.'.format(c))
